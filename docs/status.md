...
**TD-10 shipped — auth response now carries the full user shape.** Merged across all three repos on `feat/td-10-auth-user-shape`. Shared: `@localloop/shared-types@1.3.0` exports a `UserSummary` interface (`id`, `displayName`, `avatarUrl`, `dmPermission`, `provider`, `createdAt`) and both `UserSummaryDto` (auth) + `UserProfileDto` (user) `implements UserSummary`. API: `UserSummaryDto` enriched from 4 fields to 6; auth use cases unchanged (already passed the full domain `User` to `fromEntity`); two new spec assertions per token path verify the round-trip. Mobile: `User` interface collapsed to `export type User = UserSummary` (drops the four dead fields `providerId`/`geohash`/`isActive`/`lastSeenAt`); `auth.api.ts:mapToAuthResponse` deleted in favor of a direct pass-through; 4 `buildUser()` fixtures + `OnboardingScreen` inline fixture trimmed; `package.json` bumped to `^1.3.0`. 119/119 API + 146/146 mobile tests green; typechecks clean both sides. Next candidates from "Up next": HOME-5 (live presence on home cards — unblocked), HOME-7 ("Ver todos →" detail screens), HOME-8 (search), Phase 3 Slice 2 (media upload), Phase 2 Redis cache (unblocked), Phase 2 API integration tests, Maestro E2E, or remaining RQ migration backlog (TD-09).
...
**Nothing in flight.** TD-10 just shipped. Pick next from "Up next": HOME-5 (live presence on home cards — unblocked), HOME-7 ("Ver todos →" detail screens), HOME-8 (search), Phase 3 Slice 2 (media upload + picker), Phase 2 Redis cache (unblocked), Phase 2 API integration tests, Maestro E2E (Phase 1/2 flows), or remaining RQ migration backlog (TD-09).
