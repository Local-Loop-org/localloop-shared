...
**Phase 4 S1 DM inbox API landed on `feat/dm-inbox-api`** (API only). `GET /dm` returns paginated conversation threads; `GET /dm/requests` returns pending DM requests. `POST /dm/:userId` now routes to `dm_requests` instead of throwing 403 when permission requires it (NOBODY always; MEMBERS + no shared group; exception whitelist bypasses all). Migration `1717100000000-AddDmInboxSupport` adds `dm_requests`, `dm_conversation_state` (unread + archived state per user per conversation), and `dm_permission_exceptions` tables. `parseTimestampIdCursor` extracted to `cursor.utils.ts` and reused across all paginated use cases. Still open: S2 realtime inbox events (`dm_summary_update`), S3 DM push fan-out, M1 mobile inbox wiring, M2 DmChatScreen, Phase 3 Slice 3 message permissions, Phase 3 Slice 2 media upload, GroupMembersScreen redesign, HOME-12 Map, Phase 2 Redis cache. HOME-8 search remains Phase 5 Polish.
