...
**HOME-9 and HOME-10 fully merged (API + mobile) as of 2026-05-10.** HOME-9 adds full membership status on discovery cards (member, pending, banned, or none); API `DiscoverNearbyGroupsUseCase` LEFT JOINs `group_members` to derive `myRole` + `memberStatus`, shared types extend `NearbyGroup` with both fields, mobile renders four status badges and branches `handleDiscoveryPress` accordingly. HOME-10 shipping complete: API accepts configurable `radiusKm` (default 25km, max 50km), groups have per-group visibility radius, mobile has radius chip/slider UI persisted in Zustand. Next unblocked items: HOME-5 (live presence on home cards), HOME-8 (search), Phase 3 Slice 2 (media upload), Phase 2 Redis cache, or RQ migration backlog (TD-09).
...
Nothing in progress. Pick next from "Up next": HOME-5 (live presence on home cards), HOME-8 (search), Phase 3 Slice 2 (media upload), Phase 2 Redis cache, or RQ migration backlog (TD-09).
