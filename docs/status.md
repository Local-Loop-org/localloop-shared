...
**Phase 4 DM flow shipped end-to-end on mobile except for E2E.** DM-TASK-A through DM-TASK-H closed; S1/S2/S3 (API), M1, M2, M3 peer presence, M4 shipped on mobile; DM-exception-candidates contract + API live; Cluster A push tap routing/cleanup/dedup, payload metadata, and immediate per-chat digest replacement are implemented; Cluster B is fully closed (B6 bubble status icons render checkmark glyphs on own DM bubbles); Cluster C is fully closed on the mobile side — C1 + C2 + C3 + C4 + C5 + C6 + C10 (tombstone) + C11 + C12 + C13 + C14 ("editado" suffix) all shipped. Cluster E is fully closed — E1 + E2 + E3 (15s send-timeout detection) + E4 (failed-bubble visual) + E5 (tap-retry handler) + E6 (long-press "Descartar envio") all shipped on mobile. **Open DM work**: M5 Maestro E2E; Phase 5 DM polish. **Other open work**: Phase 3 Slice 3 message permissions (API enforcement + mobile gating), GroupMembersScreen redesign, Phase 3 Slice 2 media upload, HOME-12 Map, Phase 2 Redis cache.
...
2026-06-01 — Cluster E6 shipped on `feat/chat-discard-failed-send` (mobile). **Mobile**: long-press on an error-state optimistic temp now opens a sheet with a single "Descartar envio" row that drops the message from the React Query cache without firing a retry. `availableMessageActions` policy gains a fourth flag `canDiscard` and an optional `sendStatus` context field; the universal `temp-` short-circuit splits into two cases — `'error'` → discard-only, anything else → no actions (preserves prior behaviour for `'sending'` temps). `MessageActionSheet` adds a `'discard'` `MessageActionId` + `DISCARD_ITEM` row (trash icon, danger styling). Both `DmChatScreen` and `GroupChatScreen` thread `getLocalSendStatus(message)` into the policy and `handleSelectMessageAction` gains a `'discard'` branch that calls `queryClient.setQueryData(<historyKey>, (old) => removeXxxMessageById(old, id))` — pure cache removal, no API call. New idempotent `removeDirectMessageById` / `removeGroupMessageById` sibling helpers live in the existing delete-hook files alongside `markXxxMessageDeleted`; the older tombstone helpers handle the persisted-delete path (C10) and the new filter helpers handle the never-sent-temp discard path (E6). User-visible decisions: no confirm dialog (consistent with the rest of the sheet), Discard-only sheet (Retry stays on the tap icon from E5), strict `sendStatus === 'error'` gate (sending-state temps still show no sheet, avoiding a "ghost" message if a slow WS send lands after discard). No API / shared-types change. Tests: 560/560 (added 4 policy cases for the new gating + new `MessageActionSheet` test asserting the discard row renders and fires; backfilled `canDiscard:false` across existing policy + sheet fixtures). **Cluster E closes.**
...
DM flow's last open task is **M5 Maestro E2E** (zero `.yaml` flows in the mobile repo). Once that lands the DM track closes apart from Phase 5 polish. Cluster E send-failure + retry is now closed end-to-end: E3's 15s setTimeout flips a stuck temp to `'error'`; E4 renders the failed-bubble treatment + retry affordance; E5 wires tap-retry that re-emits with the same `clientMessageId` and re-arms the tracker; E6 (this PR) wires long-press "Descartar envio" so the user can drop the failed temp from the cache without retrying. Cluster C is also closed end-to-end on mobile — C1–C13 + C14 ("editado" suffix) all shipped. Phase 3 Slice 3 message permissions is mid-flight: shared enum published (step 1/3); API migration + `SendMessageUseCase` enforcement (step 2/3) and mobile composer gating (step 3/3) remain. Other candidates: GroupMembersScreen redesign, HOME-12 Map, Phase 3 Slice 2 media upload, Phase 2 Redis cache.
...
- [x] C10 — **Mobile**: tombstone bubble when `isDeleted` is true ("Mensagem apagada"). Tombstoned messages reject Reply/Edit/Delete actions and don't accept reactions (D constraint).
...
- [x] E6 — **Mobile**: long-press on an error-state bubble opens a single-row sheet with "Descartar envio" that drops the optimistic temp from the React Query cache without firing a retry. Policy split: `temp-` + `sendStatus === 'error'` → discard-only; `temp-` + anything else → no sheet. `removeDirectMessageById` / `removeGroupMessageById` filter helpers added next to the existing tombstone helpers.
