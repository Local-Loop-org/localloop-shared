...
**HOME-6 shipped — discover cards now render real distance.** All three repos merged on `feat/home-6-distance-meters`. `@localloop/geo-utils@1.2.0` exports a haversine `distanceMeters(lat1, lng1, lat2, lng2)`; `@localloop/shared-types@1.1.0` exports a shared `NearbyGroup` interface (replacing the inline mobile type). API: `groups` table gained `anchor_lat` / `anchor_lng NUMERIC(9,6) NOT NULL` (migration `1714500000000-AddGroupAnchorCoordinates`, backfilled from geohash centers via `ngeohash.decode`); `CreateGroupUseCase` persists lat/lng alongside the geohash, and `DiscoverNearbyGroupsUseCase` now returns `distanceMeters` instead of `proximityLabel`. Mobile: new `src/shared/format/distance.ts` formatter (`<1000m` → nearest-10 `M`, `<10km` → one-decimal pt-BR `Km`, `≥10km` → integer `Km`); `DiscoverCard`/`DiscoverRow` consume `formatDistance(group.distanceMeters)`. 132/132 mobile tests green, 112/112 API tests green, all typechecks clean. Next candidates from "Up next": HOME-5 (live presence — depends on Chat redesign Slice B), HOME-7 ("Ver todos →" detail screens), Chat redesign Slice B (proximity + presence), Phase 3 Slice 2 (media upload), Phase 2 API integration tests, Maestro E2E, or remaining RQ migration backlog.
