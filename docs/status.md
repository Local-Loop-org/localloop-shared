...
One migration covers all three columns (`reply_to_message_id`, `deleted_at`, `edited_at`). Long-press primitive on the bubble is shared by Delete and Edit. **Re-evaluate TD-12 before opening C** — duplicate `SendMessageUseCase` flavors still block media/edit work if left unaddressed. TD-13 is closed: realtime wiring now lives behind `RealtimeModule` and `RealtimeEventsModule`.
