...
- [x] HOME-10 API — Configurable discovery radius (`feat/home-10-radius`). `GET /groups/nearby` accepts optional `radiusKm` (default 25km, max 50km); `DiscoverNearbyGroupsUseCase` steps down geohash precision via `precisionForRadiusKm` and post-filters by `MIN(userRadiusKm, group.radiusKm)`; migration `1715000000000-AddGroupRadius` adds `radius_km NUMERIC(5,2) NOT NULL` backfilled from anchor type; `CreateGroupUseCase` defaults from `DEFAULT_RADIUS_KM_BY_ANCHOR`; `@localloop/geo-utils@1.3.0` exports `precisionForRadiusKm`; `@localloop/shared-types@1.4.0` adds `radiusKm` to `NearbyGroup`. Mobile radius UI deferred.
...
Nothing in progress. Pick next from "Up next": HOME-9 (membership status on `NearbyGroup` — unblocked), HOME-10 mobile (radius chip/slider — API already ships the param), HOME-5 (live presence on home cards), HOME-8 (search), Phase 3 Slice 2 (media upload), Phase 2 Redis cache, or RQ migration backlog (TD-09).
...
- [x] **HOME-9** API + mobile: full membership status on discovery cards. Four states — member, pending approval, banned, none. No new migration needed: `MemberStatus` enum (`ACTIVE`, `PENDING`, `BANNED`) already lives in `group_members.status`. API: `IGroupRepository.findNearby` receives `userId`; single query LEFT JOINs `group_members` on `(group_id, user_id)` to read `role` + `status`; derives `myRole: MemberRole | null` (null unless `status = ACTIVE`) and `memberStatus: MemberStatus | null`. `NearbyGroupDto` gains both fields. Shared: extend `NearbyGroup` interface with `myRole` + `memberStatus` (`@localloop/shared-types` minor bump). Mobile: `DiscoverCard` / `DiscoverRow` renders one of four status badges driven by `memberStatus`: `ACTIVE` → "Conversar", `PENDING` → "Aguardando aprovação", `BANNED` → "Banido", `null` → "entrar". `handleDiscoveryPress` branches: `ACTIVE` → navigate directly with real `myRole`; `PENDING` → do nothing(Aguardando aprovação gives enough information); `BANNED` → do nothing; `null` → existing join flow.
- [x] **HOME-10 API** — `GET /groups/nearby` accepts `radiusKm` (default 25, max 50); per-group `radius_km` column; filter intersects both radii. _Done (`feat/home-10-radius`)._
- [ ] **HOME-10 mobile** — radius chip/slider UI on `HomeScreen` (or a filter bottom sheet); persisted to Zustand (client-only preference, not a server profile field). API already ships the `radiusKm` param and `NearbyGroup.radiusKm` response field.
