...
**TD-10 — auth-response/User-shape drift fix in flight on `feat/td-10-auth-user-shape`.** Aligns the `POST /auth/google` / `POST /auth/apple` response with the full user shape so mobile no longer fakes fields. Shared: `@localloop/shared-types@1.3.0` exports a `UserSummary` interface (`id`, `displayName`, `avatarUrl`, `dmPermission`, `provider`, `createdAt`); both `UserSummaryDto` (auth) and `UserProfileDto` (user) `implements UserSummary`. API: `UserSummaryDto` enriched from 4 fields to 6 — `dmPermission` and `createdAt` (ISO) added to `fromEntity` (already had access to the full domain `User`); two new spec assertions per token use case verify the round-trip. Mobile: `User` interface replaced with `export type User = UserSummary` (drops the four dead fields `providerId`/`geohash`/`isActive`/`lastSeenAt` — Phase 1 audit confirmed nothing read them); `auth.api.ts` collapses `mapToAuthResponse` to a direct pass-through (the entire fakes block is gone); 4 `buildUser()` test fixtures + the `OnboardingScreen` inline fixture trimmed to the new shape. 21/21 API suites (119/119 tests) and 21/21 mobile suites (146/146 tests) green; both typechecks clean. PRs open across api + mobile + shared on `feat/td-10-auth-user-shape`; shared must publish first so API + mobile can resolve `^1.3.0` in CI.

## Last updated

2026-05-03 — TD-10 resolution pushed across all three repos on `feat/td-10-auth-user-shape`. Shared adds `UserSummary` to `@localloop/shared-types@1.3.0`. API enriches `UserSummaryDto` with `dmPermission` + `createdAt` (and tags `UserProfileDto` `implements UserSummary` for type-level enforcement); auth use cases unchanged because they already passed the full domain `User` to `fromEntity`. Mobile collapses the local 10-field `User` interface to a re-export of `UserSummary`, deletes the 6-line fakes block in `mapToAuthResponse`, and trims 4 `buildUser()` fixtures + 1 inline fixture. Plan captured in `~/.claude/plans/lets-resolve-the-td-10-piped-sunrise.md`.
...
**TD-10 — auth-response/User-shape drift fix** on `feat/td-10-auth-user-shape` (api + mobile + shared). All commits pushed; PRs open. Shared must merge + publish `@localloop/shared-types@1.3.0` first so API and mobile can resolve the new `UserSummary` interface in CI. After merge, candidates from "Up next": HOME-5 (live presence on home cards — unblocked), HOME-7 ("Ver todos →" detail screens), HOME-8 (search), Phase 3 Slice 2 (media upload + picker), Phase 2 Redis cache (unblocked), Phase 2 API integration tests, Maestro E2E (Phase 1/2 flows), or remaining RQ migration backlog (TD-09).
