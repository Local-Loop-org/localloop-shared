...
**GroupDetailScreen fully revamped as of 2026-05-11.** G2 Expressive redesign (Hero gradient/glow/border, MembersSection row list with expand drawer, MemberStack border fix), full React Query migration (9 new hooks — useGroupDetail, useGroupJoinRequests, useGroupMembers, useLeaveGroup, useDeleteGroup, useBanMember, useResolveJoinRequest, useUpdateGroup, plus useJoinGroup extracted earlier), and inline editing for owners/moderators (pencil in HeaderBar → all fields live → Salvar/Cancelar). API `PATCH /groups/:id` (`UpdateGroupUseCase`) allows owners and moderators to update name, description, anchorLabel, privacy, radiusKm. 214/214 tests, typecheck clean. Next unblocked items: HOME-5 (live presence on home cards), HOME-8 (search), Phase 3 Slice 2 (media upload), Phase 2 Redis cache, Phase 3 Slice 3 (message permissions).
...
Nothing in progress. Pick next from "Up next": HOME-5 (live presence on home cards), HOME-8 (search), Phase 3 Slice 2 (media upload), Phase 2 Redis cache, Phase 3 Slice 3 (message permissions), or `GroupMembersScreen` RQ migration (TD-09 remaining).
...
- [ ] **HOME-5** Mobile: live presence on home cards (depends on Chat redesign Slice B presence pipeline). Adds green dot + live count badge + "Online Agora" subtitle.
